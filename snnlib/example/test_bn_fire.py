...
if input_type == 'fc':
    sop_obj = sop.BnFire(ut.cin, (0,1)).cuda()
    gop_obj = gop.BnFire(ut.cin, 'fc').cuda()
elif input_type == 'conv':
    sop_obj = sop.BnFire(ut.cin, (0,1,3,4)).cuda()
    gop_obj = gop.BnFire(ut.cin, 'conv').cuda()
...
if input_type == 'fc':
    ...
    _, gop_result = gop_obj(ut.gold_x_fc)
elif input_type == 'conv':
    sop_result = sop_obj(ut.x_conv)
    _, gop_result = gop_obj(ut.gold_x_conv)
...
ut.check_diff(sop_final, gop_final, 'bn fire fp')
ut.check_diff(sop_x_grad, gop_x_grad, 'bn fire bp')
