...
class SyncBatchnormFunction(Function):
    ...
    @staticmethod
    def forward(ctx, input, weight, bias, running_mean, running_variance, eps, process_group, world_size,k):
        ...
        k1 = torch.zeros(1).to(running_variance.device)
        k1[0]=k
        ctx.save_for_backward(c_last_input, weight, bias,
                              running_mean, running_variance,k1)
        ctx.eps = eps
    ...
    def backward(ctx, grad_output):
        ...
        c_last_input, weight, bias, running_mean, running_variance,k = ctx.saved_tensors
        ...
        if ctx.needs_input_grad[0]:
            ...
            c_last_grad_input = (c_last_grad - mean_dy - (c_last_input - running_mean) / (
                running_variance + eps) * mean_dy_xmu*k) / torch.sqrt(running_variance + eps)
            if weight is not None:
